...
//    당시 프로젝트가 Hobby라 sub-daily 크론이 배포를 막아 vercel.json 에서 빠졌고(커밋 b2f6074),
//    대체가 안 들어가 예약편지가 봉인(sealed)된 채 영영 도착 안 하는 사고가 났다(꼼이 생일편지).
//    지금 kkommorning-v2 는 Pro 팀(tonbonamis-projects) 소속이라 sub-daily 크론이 된다 → 2026-09
//    notify-pending-letters(*/5)를 '주 경로'로 되살렸다(정시 도착). 이 승격 로직은 그와 별개로
//    '이미 매일 도는 크론 4개'에도 얹어 백스톱을 둔다(*/5 크론이 또 빠져도 하루 4윈도우로 도착 보장).
//    두 경로가 겹쳐 돌아도 notifiedAt 도장으로 멱등 → 중복 푸시 없음.
//
